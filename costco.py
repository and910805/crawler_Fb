...
class Connect:
    ...
    def connect_pygsheets(self):
        ...
        self.spreadsheet = self.gc.open_by_key('1WH58sY3Vn6FSKCkSHpDA2F-DSWOkN27b0UcPih9yJj0') 
        self.worksheet = self.spreadsheet.sheet1
        
    def runsrc(self):#連上網址
        self.browser.get(index_url)
        time.sleep(3)
     
    def print_all(self):#印出文章
        print("內文: "+ self.contents)
        print("按讚數: "+self.good_number)
        print("發文時間: "+self.posttime)
        print("留言數量: "+str(self.comment_number))
    def content(self):#開始操作
            ...
            if self.comment_number > 50:#只抓大於50的貼文
                ...
                for tmp2 in posts[1:]:#要找尋剛剛是抓哪一篇文章
                    if(tmp_number==post_number):
                        usercontents = tmp2.find_all('div',{'class':'xdj266r x11i5rnm xat24cr x1mh8g0r x1vvkbs x126k92a'})
                        ...
                        for div, people in zip(comments_div[1:], comments_people):
                            try:
                                ...
                                self.worksheet.update_value((a, 5), self.comment)
                                a+=1
